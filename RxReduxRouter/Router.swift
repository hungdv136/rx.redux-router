...
open class Router<State: StateType> {
    ...
    public init(store: Store<State>, rootRoutable: Routable, stateSelector: @escaping NavigationStateSelector) {
        ...
        store.state.map { stateSelector($0) }
        .distinctUntilChanged { RouteHash(route: $0.0.route) == RouteHash(route: $0.1.route) }
            ...
            waitForRoutingCompletionQueue.async {
                ...
                case let .pop(responsibleRoutableIndex, segmentToBePopped):
                    DispatchQueue.main.async {
                        self.routables[responsibleRoutableIndex].popRouteSegment(segmentToBePopped, animated: state.changeRouteAnimated) {
                            semaphore.signal()
                        }
                ...
                case let .change(responsibleRoutableIndex, segmentToBeReplaced, newSegment):
                    DispatchQueue.main.async {
                        self.routables[responsibleRoutableIndex + 1] =
                            self.routables[responsibleRoutableIndex].changeRouteSegment(segmentToBeReplaced, to: newSegment, animated: state.changeRouteAnimated) {
                                semaphore.signal()
                        }
                    }
                ...
                case let .push(responsibleRoutableIndex, segmentToBePushed):
                    DispatchQueue.main.async {
                        self.routables.append(
                            self.routables[responsibleRoutableIndex].pushRouteSegment(segmentToBePushed, animated: state.changeRouteAnimated) {
                                semaphore.signal()
                            }
                        )
                    }
                ...
                if case .timedOut = result {
                    print("[RxReduxRouter]: Router is stuck waiting for a" +
...
func RxReduxRouterStuck() {}

enum RoutingActions {
    case push(responsibleRoutableIndex: Int, segmentToBePushed: String)
    case pop(responsibleRoutableIndex: Int, segmentToBePopped: String)
    case change(responsibleRoutableIndex: Int, segmentToBeReplaced: String, newSegment: String)
}
